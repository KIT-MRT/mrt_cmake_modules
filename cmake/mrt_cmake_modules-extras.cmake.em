...
#handle pure dynamic reconfigure files
    foreach (_cfg ${ARGN})
        get_filename_component(_cfgext ${
        _cfg} EXT)
        if( _cfgext STREQUAL ".cfg" )
            list(APPEND _${
        PROJECT_NAME}_pure_cfg_files "${_cfg}")
        else()
            list(APPEND _${
        PROJECT_NAME}_rosparam_other_param_files "${_cfg}")
        endif()
    endforeach()
#generate dynamic reconfigure files if rosinterface is not here
    if(_${
        PROJECT_NAME}_pure_cfg_files AND NOT TARGET ${
        PROJECT_NAME}_gencfg AND NOT rosinterface_handler_FOUND_CATKIN_PROJECT)
    ...
    if(_${
        PROJECT_NAME}_rosparam_other_param_files AND NOT rosinterface_handler_FOUND_CATKIN_PROJECT)
...
#handle pure dynamic reconfigure files
    foreach (_cfg ${ARGN})
        ...
        if(NOT _cfgext STREQUAL ".cfg" )
            list(APPEND _${
        PROJECT_NAME}_rosif_other_param_files "${_cfg}")
        endif()
    endforeach()
    if(_${
        PROJECT_NAME}_rosif_other_param_files AND NOT rosparam_handler_FOUND_CATKIN_PROJECT)
        message(FATAL_ERROR "Dependency rosinterface_handler or rosparam_handler could not be found. Did you add it to your package.xml?")
    endif()
endmacro()
...
#we need this only once per project
    if(MRT_NO_FAIL_ON_TESTS OR _mrt_checks_${
        PROJECT_NAME} OR NOT TARGET run_tests)
        return()
    endif()
    ...
    set(_mrt_checks_${
        PROJECT_NAME} TRUE PARENT_SCOPE)
endmacro()
...
#and installed from the resulting library.If this project declares any libraries \
    with ``mrt_add_library()``, they will automatically be linked with this library.
#
#This function will define the compiler variable ``PYTHON_API_MODULE_NAME`` with the name of the generated \
    library.This can be used in the ``BOOST_PYTHON_MODULE`` C++ Macro.
#
...
#mark that catkin_python_setup() was called and the setup.py file contains a package with the same name as the current \
                                   project
#in order to disable installation of generated __init__.py files in \
    generate_messages() and generate_dynamic_reconfigure_options()
        set(${
        PROJECT_NAME}_CATKIN_PYTHON_SETUP_HAS_PACKAGE_INIT TRUE PARENT_SCOPE)
    endif()
    if(${
        PROJECT_NAME}_PYTHON_API_TARGET)
...
#add library as target
    ...
    target_compile_definitions(${
        TARGET_NAME} PRIVATE -DPYTHON_API_MODULE_NAME=lib${LIBRARY_NAME})
...
#Adds a library.
#
#This command ensures the library is compiled with all necessary dependencies.If no files are passed, \
    the command will return silently.
#
#..note::Make sure to call this after all messages and \
    parameter generation CMAKE - Commands so that all dependencies are visible.
#
#The files are automatically added to the list of installable targets so that ``mrt_install`` can mark \
...
# : param DEPENDS : List of extra(non - catkin, \
                                  non - mrt)    \
                                      dependencies.This should only be required for including external projects.
# : type DEPENDS : list of strings
# : param LIBRARIES : Extra(non - catkin, \
                            non - mrt) libraries to link to.This should only be required for external projects
...
#we only set a fake target to make the files show up in IDEs
        ...
        add_custom_target(${
        LIBRARY_TARGET_NAME} SOURCES ${MRT_ADD_LIBRARY_INCLUDES})
        return()
    endif()
...
#generate the target
    ...
    add_dependencies(${
        ...
        _EXPORTED_TARGETS} ${MRT_ADD_LIBRARY_DEPENDS})
    target_link_libraries(${
        LIBRARY_TARGET_NAME}
        ${
        catkin_LIBRARIES}
...
#add dependency to python_api if existing(needs to be declared before this library)
    if(${
        ...
        _PYTHON_API_TARGET} ${LIBRARY_TARGET_NAME})
    endif()
...
#append to list of all targets in this project
    set(${
        ...
        _GENERATED_LIBRARIES} ${
        LIBRARY_TARGET_NAME} PARENT_SCOPE)
    set(${
        PROJECT_NAME}_MRT_TARGETS ${
        $ {
            PROJECT_NAME
...
# : param FOLDER : Folder containing the.cpp /.cc - files and.h /.hh /.hpp files for the executable, \
...
#generate the target
    ...
    add_dependencies(${
        ...
        _EXPORTED_TARGETS} ${MRT_ADD_EXECUTABLE_DEPENDS})
    target_link_libraries(${
        EXEC_TARGET_NAME}
        ${
        catkin_LIBRARIES}
...
#Adds a nodelet.
#
#This command ensures the nodelet is compiled with all necessary dependencies.Make sure to add lib{ \
    NAME } _nodelet to the ``nodelet_plugins.xml`` file.
#
#..note::Make sure to call this after all messages and \
    parameter generation CMAKE - Commands so that all dependencies are visible.
#
...
#The command will look for a ``* _node.cpp``- \
    file and remove it from the list of files to avoid ``main()``- functions to be compiled into the library.
#
# : param nodeletname : base name of the nodelet( \
    _nodelet will be appended to the base name to avoid conflicts with library packages)
# : type nodeletname : string
# : param FOLDER : Folder with cpp files for the executable, relative to ``${CMAKE_CURRENT_LIST_DIR }``
# : type FOLDER : string
# : param DEPENDS : List of extra(non - catkin, \
                                  non - mrt)    \
                                      CMAKE dependencies.This should only be required for including external projects.
# : type DEPENDS : list of strings
# : param LIBRARIES : Extra(non - catkin, \
                            non - mrt) libraries to link to.This should only be required for external projects
# : type LIBRARIES : list of strings
...
#generate the target
    ...
    add_dependencies(${
        ...
        _EXPORTED_TARGETS} ${MRT_ADD_NODELET_DEPENDS})
    target_link_libraries(${
        NODELET_TARGET_NAME}
        ${
        catkin_LIBRARIES}
...
#append to list of all targets in this project
    set(${
        ...
        _GENERATED_LIBRARIES} ${
        NODELET_TARGET_NAME} PARENT_SCOPE)
    set(${
        PROJECT_NAME}_MRT_TARGETS ${
        $ {
            PROJECT_NAME
...
#
#This command ensures the node /                                               \
    nodelet are compiled with all necessary dependencies.Make sure to add lib{ \
        NAME } _nodelet to the ``nodelet_plugins.xml`` file.
#
#..note::Make sure to call this after all messages and \
    parameter generation CMAKE - Commands so that all dependencies are visible.
...
#The files are automatically added to the list of installable targets so that ``mrt_install`` can mark \
    them for installation.
#
#It requires a ``* _nodelet.cpp`` file and a ``* _node.cpp`` file to be present in this folder.It will then compile a \
        nodelet -                                                                                                     \
    library,                                                                                                          \
    create an executable from the ``* _node.cpp`` file and link the executable with the nodelet library.
#
#Unless the variable ``${MRT_NO_FAIL_ON_TESTS }`` is set, failing unittests will result in a failed build.
#
# : param basename : base name of the node / \
    nodelet(_nodelet will be appended for the nodelet name to avoid conflicts with library packages)
...
#pass lists on to parent scope
    set(${
        ...
        _GENERATED_LIBRARIES} PARENT_SCOPE)
    set(${
        PROJECT_NAME}_MRT_TARGETS ${
        $ {
            PROJECT_NAME
        }
        _MRT_TARGETS} PARENT_SCOPE)

#check if a target was added
    if(NOT TARGET ${
        NODELET_TARGET_NAME} OR DEFINED MRT_SANITIZER_ENABLED)
...
#find node files and add them as executable
    ...
    if(NODE_CPP)
        ...
        NODELET_TARGET_NAME}
            LIBRARIES ${
        MRT_ADD_NN_LIBRARIES} ${NODELET_TARGET_NAME}
            )
#pass lists on to parent scope
        set(${
        PROJECT_NAME}_GENERATED_LIBRARIES ${
...
#Unittests can be run with "catkin run_tests" or                                      \
    similar."-test" will be appended to the name of the test node to avoid conflicts( \
        i.e.the type argument should then be < test... type = "mytest-test" / > in a mytest.test file).
#
#Unittests will always be executed with the folder as cwd.E.g.if the test folder contains a sub - folder "test_data", \
    it can simply be accessed as "test_data".
#
#If coverage information is enabled(             \
    by setting ``MRT_ENABLE_COVARAGE`` to true), \
    coverage analysis will be performed after unittests have run.The results can be found in the package's build folder in the folder "coverage".
#
#Unless the variable ``${MRT_NO_FAIL_ON_TESTS }`` is set, failing unittests will result in a failed build.
#
...
function(mrt_add_ros_tests folder)
    ...
    foreach(_ros_test ${_ros_tests})
        get_filename_component(_test_name ${
        _ros_test} NAME_WE)
...
#look for a matching.cpp
        if(EXISTS "${CMAKE_CURRENT_LIST_DIR}/${TEST_FOLDER}/${_test_name}.cpp")
            ...
            add_rostest_gtest(${
        TEST_TARGET_NAME} ${
        _ros_test} "${TEST_FOLDER}/${_test_name}.cpp")
            target_compile_options(${
        TEST_TARGET_NAME}
        ...
        _MRT_TARGETS} ${MRT_ADD_ROS_TESTS_DEPENDS}
                )
...
#exclude cpp files with a test file(those are ros tests)
        if(NOT EXISTS "${CMAKE_CURRENT_LIST_DIR}/${TEST_FOLDER}/${_test_name}.test")
            ...
            catkin_add_gtest(${
        TEST_TARGET_NAME} ${
        _test} WORKING_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}/${TEST_FOLDER})
            target_link_libraries(${
        TEST_TARGET_NAME}
                ${
        $ {
        ...
        _MRT_TARGETS} ${MRT_ADD_TESTS_DEPENDS}
                )
            set(TARGET_ADDED True)
        endif()
    endforeach()
...
function(mrt_add_nosetests folder)
    ...
    catkin_add_nosetests(${
        TEST_FOLDER}
        DEPENDENCIES ${
        MRT_ADD_NOSETESTS_DEPENDENCIES} ${
        $ {
            PROJECT_NAME
        }
        _EXPORTED_TARGETS} ${${PROJECT_NAME}_PYTHON_API_TARGET}
        )
    _mrt_register_test()
endfunction()
...
#All targets added by the mrt_add_ < library / executable / nodelet / ... >        \
    commands will be installed automatically when using this command.Other files / \
        folders(launchfiles, scripts) need to be specified explicitly.
#Non existing files and folders will be silently ignored.All files will be marked as project files for IDEs.
#
# : param PROGRAMS : List of all folders and \
    files that are programs(python scripts will be indentified and treated separately).Files will be made executable.
...
#helper function for installing programs
    function(mrt_install_program program_path)
        get_filename_component(extension ${
        program_path} EXT)
...
#make it show up in IDEs
        ...
        add_custom_target(${
        CUSTOM_TARGET_NAME} SOURCES ${program_path})
    endfunction()
...
#make them show up in IDEs
        ...
        elseif(EXISTS ${CMAKE_CURRENT_LIST_DIR}/${ELEMENT})
            ...
            install(FILES ${
        ELEMENT} DESTINATION ${CATKIN_PACKAGE_SHARE_DESTINATION})
            STRING(REGEX REPLACE "/" "-" CUSTOM_TARGET_NAME ${PROJECT_NAME}-${ELEMENT})
            add_custom_target(${
        CUSTOM_TARGET_NAME} SOURCES ${ELEMENT})
        endif()
