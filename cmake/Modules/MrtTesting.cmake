function(mrt_init_testing)
    ...
    if(NOT TARGET run_tests_${PROJECT_NAME})
        ...
        add_custom_target(_run_tests_${PROJECT_NAME}) # just to make catkin happy
        add_dependencies(run_tests run_tests_${PROJECT_NAME})
    endif()
    add_custom_target(tests_${PROJECT_NAME})
    set(MRT_TEST_RESULTS_DIR ${CMAKE_BINARY_DIR}/test_results CACHE INTERNAL "basedir where test xmls will be written (in subfolder <project_name>)")
    if(MRT_ENABLE_COVERAGE)
        ...
        set(MRT_COVERAGE_DIR ${CMAKE_BINARY_DIR}/mrt_coverage CACHE INTERNAL "")
        set(coverage_dir ${MRT_COVERAGE_DIR})
        message(STATUS "Outputting coverage to ${coverage_dir}")
    endif()
    if(NOT TARGET init_tests)
        set(pre_test_cmd "${MRT_CMAKE_MODULES_ROOT_PATH}/scripts/init_coverage.py" ${PROJECT_NAME} ${CMAKE_BINARY_DIR} ${MRT_TEST_RESULTS_DIR} ${coverage_dir})
        ...
        set(post_test_cmd "${MRT_CMAKE_MODULES_ROOT_PATH}/scripts/eval_coverage.py" ${CMAKE_SOURCE_DIR} ${CMAKE_BINARY_DIR} ${MRT_TEST_RESULTS_DIR} ${coverage_dir} ${show_result})
        if(NOT MRT_NO_FAIL_ON_TEST)
            set(fail_on_test --fail_on_test)
        endif()
        add_custom_target(eval_tests
        ...
        add_test(build_tests ${CATKIN_ENV} bash -c "\"${CMAKE_COMMAND}\" --build ${CMAKE_BINARY_DIR} --target tests && ${pre_test_cmd}")
...
function(_mrt_run_test target_name working_dir result_xml_path)
    ...
    set(run_test_cmd ${CATKIN_ENV} ${PYTHON_EXECUTABLE}
      ${MRT_CMAKE_MODULES_ROOT_PATH}/scripts/run_test.py ${result_xml_path} ${working_dir_arg} ${coverage_dir_arg} ${redirect_arg})
    ...
    set_tests_properties(${target_name} PROPERTIES DEPENDS build_tests_${PROJECT_NAME})

    # for the run_tests target the command needs to return zero so that testing is not aborted
    add_custom_target(run_test_${target_name}
...
function(_mrt_create_executable_gtest target file)
    if(NOT TARGET gtest_main)
        ...
        find_file(gtest_sources "gtest.cc"
                  PATH_SUFFIXES "../src/googletest/googletest/src"
                  )
...
function(mrt_add_gtest target file)
    ...
    set(cmd "$<TARGET_FILE:${target}> --gtest_output=xml:${result_xml_path}")
    if(MRT_ENABLE_COVERAGE)
...
function(mrt_add_rostest target launch_file)
    ...
    set(cmd "${PYTHON_EXECUTABLE} ${ROSTEST_EXE} --pkgdir=${PROJECT_SOURCE_DIR} --package=${PROJECT_NAME} --results-filename ${test_name}.xml --results-base-dir \"${MRT_TEST_RESULTS_DIR}\" ${CMAKE_CURRENT_LIST_DIR}/${launch_file}")
...
function(_mrt_add_nosetests_impl folder)
    ...
    set(test_name ${PROJECT_NAME}-nosetests-${output_file_name})
    ...
    if(MRT_ENABLE_COVERAGE)
        ...
        execute_process(COMMAND find ${path} -name *.py -type f -not -executable
            OUTPUT_VARIABLE test_py_files
            )
        execute_process(COMMAND find ${path} -name *.py -type f -executable
            OUTPUT_VARIABLE exec_py_files
            )
        if(has_init_py AND (test_py_files OR NOT exec_py_files))
            find_program(PY_COVERAGE NAMES
                "python-coverage${PYTHON_VERSION_MAJOR}.${PYTHON_VERSION_MINOR}"
                "python-coverage${PYTHON_VERSION_MAJOR}"
                "python-coverage")
            if(PY_COVERAGE)
                message(STATUS "Enabling coverage for python tests")
                set(covarg " --with-coverage --cover-erase --cover-inclusive --cover-package=src/${PROJECT_NAME} --cover-xml --cover-xml-file=\"${MRT_COVERAGE_DIR}/${test_name}_coverage.xml\"")
            endif()
        endif()
    ...
    set(result_xml_path ${output_path}/nosetests-${output_file_name}.xml)
    set(cmd "${NOSETESTS} -P --debug-log=sys.stdout --process-timeout=${_nose_TIMEOUT} ${tests} --with-xunit --xunit-file=\"${result_xml_path}\" ${covarg}")
    _mrt_run_test(${test_name} ${CMAKE_CURRENT_LIST_DIR} ${result_xml_path} ${coverage_arg} COMMAND ${cmd} ${cover_dir_arg} REDIRECT_STDERR)
    if(ARG_DEPENDS)
    ...
    if(${PROJECT_NAME}_PYTHON_API_TARGET)
        ...
        add_test(build_${test_name} "${CMAKE_COMMAND}" --build ${CMAKE_CURRENT_BINARY_DIR} --target ${${PROJECT_NAME}_PYTHON_API_TARGET})
        set_tests_properties(${test_name} PROPERTIES DEPENDS build_tests_${PROJECT_NAME})
    endif()
endfunction()
