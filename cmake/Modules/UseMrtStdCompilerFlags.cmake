# this file creates the target compiler_flags and private_compiler_flags that will be used by all targets created by mrt_cmake_modules

# export compile commands
set(CMAKE_EXPORT_COMPILE_COMMANDS YES)

add_library(compiler_flags INTERFACE)
add_library(private_compiler_flags INTERFACE)

# by default, at least c++14 is required to build externally, c++17 to build internally
if(CMAKE_VERSION VERSION_LESS "3.8")
    target_compile_options(compiler_flags INTERFACE -std=c++14)
    target_compile_options(private_compiler_flags INTERFACE -std=c++14)
else()
    target_compile_features(compiler_flags INTERFACE cxx_std_14)
    target_compile_features(private_compiler_flags INTERFACE cxx_std_17)
endif()

# Add _DEBUG and _GLIBCXX_ASSERTIONS for debug configuration. This enables e.g. assertions in OpenCV and the STL.
if (CMAKE_VERSION VERSION_GREATER "3.12")
    target_compile_definitions(compiler_flags INTERFACE $<$<CONFIG:Debug>:_DEBUG> $<$<CONFIG:Debug>:_GLIBCXX_ASSERTIONS>)
endif()

# Add support for std::filesystem. For GCC version <= 8 one needs to link against -lstdc++fs.
target_link_libraries(compiler_flags INTERFACE $<$<AND:$<CXX_COMPILER_ID:GNU>,$<VERSION_LESS:$<CXX_COMPILER_VERSION>,9.0>>:stdc++fs>)
...
if(MRT_ENABLE_COVERAGE)
    include(MRTCoverage)
    target_compile_options(private_compiler_flags INTERFACE
        $<${gcc_like_cxx}:-g;--coverage>
        $<${gcc_like_c}:-g;--coverage>
        )
    target_link_options(private_compiler_flags INTERFACE
...
message("Error flags: ${MRT_USE_DEFAULT_WERROR_FLAGS}")
...
# Include config file if set. This is done last so that the target compiler_flags can be further modified
