...
if(NOT TARGET mrt_sanitizer_lib_flags AND NOT TARGET mrt_sanitizer_exe_flags)
    add_library(mrt_sanitizer_lib_flags INTERFACE)
    add_library(mrt_sanitizer_exe_flags INTERFACE)
    ...
    if(MRT_SANITIZER STREQUAL "checks" OR MRT_SANITIZER STREQUAL "check_race")
        target_compile_options(mrt_sanitizer_lib_flags INTERFACE
        ...
        target_compile_options(mrt_sanitizer_exe_flags INTERFACE
        ...
        target_link_options(mrt_sanitizer_lib_flags INTERFACE
            $<$<AND:${gcc_cxx},$<STREQUAL:${MRT_SANITIZER},checks>>:-fsanitize=undefined,bounds-strict,float-divide-by-zero,float-cast-overflow;-fsanitize-recover=alignment>
            $<$<AND:${gcc_cxx},$<STREQUAL:${MRT_SANITIZER},check_race>>:-fsanitize=thread,undefined,bounds-strict,float-divide-by-zero,float-cast-overflow>
            )
        target_link_options(mrt_sanitizer_exe_flags INTERFACE
            $<$<AND:${gcc_cxx},$<STREQUAL:${MRT_SANITIZER},checks>>:-fsanitize=address,leak,undefined,bounds-strict,float-divide-by-zero,float-cast-overflow;-fsanitize-recover=alignment>
            $<$<AND:${gcc_cxx},$<STREQUAL:${MRT_SANITIZER},check_race>>:-fsanitize=thread,undefined,float-divide-by-zero,float-cast-overflow>
            )
...
function(mrt_add_links target)
    cmake_parse_arguments(ARG "CUDA;NO_SANITIZER" "" "" ${ARGN})
    ...
    if(NOT ARG_NO_SANITIZER)
        if(target_type STREQUAL "EXECUTABLE")
            if(TARGET mrt_sanitizer_exe_flags)
                target_link_libraries(${target} PRIVATE mrt_sanitizer_exe_flags)
            endif()
        elseif(NOT target_type STREQUAL "INTERFACE_LIBRARY")
            if(TARGET mrt_sanitizer_lib_flags)
                target_link_libraries(${target} PRIVATE mrt_sanitizer_lib_flags)
            endif()
        endif()
    endif()
    ...
    if(NOT target_type STREQUAL "INTERFACE_LIBRARY")
        ...
        if(TARGET private_compiler_flags)
            target_link_libraries(${target} PRIVATE private_compiler_flags)
        endif()
    else()
        if(TARGET compiler_flags)
            target_link_libraries(${target} INTERFACE compiler_flags)
        endif()
...
function(mrt_add_library libname)
    ...
    if(NOT MRT_ADD_LIBRARY_SOURCES)
        ...
        foreach(header ${MRT_ADD_LIBRARY_INCLUDES})
            # it is ok if a dir is set multiple times
            get_filename_component(include_dir ${header} DIRECTORY)
            target_include_directories(${LIBRARY_TARGET_NAME} PRIVATE ${include_dir})
        endforeach()
...
function(mrt_add_ros_tests folder)
    ...
    foreach(_ros_test ${_ros_tests})
        ...
        if(EXISTS "${PROJECT_SOURCE_DIR}/${TEST_FOLDER}/${_test_name}.cpp")
            ...
            mrt_add_links(${TEST_TARGET_NAME})
