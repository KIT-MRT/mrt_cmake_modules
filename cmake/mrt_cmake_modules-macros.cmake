...
function(mrt_add_library libname)
    ...
    if(NOT MRT_ADD_LIBRARY_SOURCES)
        ...
        set(public INTERFACE) # needed for target_include_directories
    else()
    ...
    # set the include dir for installation and dependent targets. This is hardcoded to the "include"-folder of the project, because catkins devel/symlink feature partly relys on that.
...
function(mrt_add_executable execname)
    ...
    foreach(include ${${PROJECT_NAME}_LOCAL_INCLUDE_DIRS})
        if(EXISTS ${include})
            target_include_directories(${EXEC_TARGET_NAME} PRIVATE ${include})
        endif()
    endforeach()
    if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/include/${PROJECT_NAME})
        target_include_directories(${EXEC_TARGET_NAME} PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/include/${PROJECT_NAME})
    endif()
    ...
    if (_MRT_CUDA_SOURCE_FILES)
        ...
        foreach(include ${${PROJECT_NAME}_LOCAL_INCLUDE_DIRS})
            if(EXISTS ${include})
                target_include_directories(${CUDA_TARGET_NAME} PRIVATE ${include})
            endif()
        endforeach()
        if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/include/${PROJECT_NAME})
            target_include_directories(${CUDA_TARGET_NAME} PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/include/${PROJECT_NAME})
        endif()
...
function(mrt_add_ros_tests folder)
    ...
    foreach(_ros_test ${_ros_tests})
        ...
        if(EXISTS "${PROJECT_SOURCE_DIR}/${TEST_FOLDER}/${_test_name}.cpp")
            ...
            endforeach()
            if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/include/${PROJECT_NAME})
                # unittests generally expect they can include headers like the library can
                target_include_directories(${TEST_TARGET_NAME} PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/include/${PROJECT_NAME})
            endif()
